...
$existingPids = @()
foreach ($scanPort in 8777..8787) {
    $existingPids += Get-NetTCPConnection -LocalPort $scanPort -ErrorAction SilentlyContinue |
...
foreach ($ownerPid in $existingPids) {
    try {
        ...
        Write-Host "Stopping old local config server on port (PID $ownerPid)."
