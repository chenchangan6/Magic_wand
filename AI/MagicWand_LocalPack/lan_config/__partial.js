(() => {
  ...
  function activeRoomId() {
    return String(state.localState?.active_room_id || state.currentRoomId || roomList()[0]?.id || '');
  }
  ...
  async function endRoom() {
    ...
    const record = {
      ...
      group_ids: controllerGroups().map((group) => group.id),
      notes: room.notes || '',
      updated_at: nowIso()
    };
  ...
  function renderWizardPage() {
    ...
    const footerBtnBase = 'inline-flex h-8 shrink-0 min-w-[96px] items-center justify-center gap-1.5 whitespace-nowrap rounded-full border text-[11px] font-bold transition hover:brightness-105 active:translate-y-px';
    const footerBtnPrimary = 'inline-flex h-8 shrink-0 min-w-[108px] items-center justify-center gap-1.5 whitespace-nowrap rounded-full border-0 text-[11px] font-extrabold text-white transition hover:brightness-105 active:translate-y-px';
    ...
    const renderGroupPick = (group, kind) => {
      ...
      const accent = kind === 'source'
        ? 'peer-checked:border-[#4ba9ff] peer-checked:bg-[rgba(20,34,52,0.98)] peer-checked:shadow-[0_0_0_1px_rgba(75,169,255,0.18),0_12px_24px_rgba(0,0,0,0.18)]'
        : 'peer-checked:border-[#6be29d] peer-checked:bg-[rgba(18,36,30,0.98)] peer-checked:shadow-[0_0_0_1px_rgba(107,226,157,0.18),0_12px_24px_rgba(0,0,0,0.18)]';
      const accentText = kind === 'source' ? 'peer-checked:text-[#d8ebff]' : 'peer-checked:text-[#dcffe8]';
      const accentDot = kind === 'source' ? 'peer-checked:bg-[#4ba9ff]' : 'peer-checked:bg-[#6be29d]';
      return `
        <label class="${[
          ...
          <div class="flex items-start gap-2">
            ...
            <span class="min-w-0">
              <span class="${['flex items-center gap-2 font-bold text-[#f3f7ff] transition', accentText].join(' ')}">
                <span class="block truncate">${escapeHtml(group.name)}</span>
                <span class="${checked ? 'inline-flex items-center rounded-full border border-[rgba(107,226,157,0.24)] bg-[rgba(107,226,157,0.14)] px-1.5 py-0.5 text-[9px] font-extrabold text-[#b6f5c8]' : 'hidden'}">已选</span>
              </span>
            ...
            <div class="rounded-[20px] border border-[rgba(88,116,154,0.26)] bg-[linear-gradient(180deg,rgba(21,30,43,0.96),rgba(17,24,36,0.94))] p-4 shadow-[0_18px_40px_rgba(0,0,0,0.22)]">
              ...
              <div class="mt-4 flex flex-nowrap items-center justify-between gap-2 overflow-x-auto border-t border-[rgba(88,116,154,0.16)] pt-3">
                <div class="flex flex-nowrap gap-2">
                  <button class="${footerBtnBase} border-[rgba(88,116,154,0.28)] bg-[rgba(24,33,47,0.92)] px-3 text-[#dbe5f4] ${step === 0 ? 'opacity-40 pointer-events-none' : ''}" type="button" data-action="wizard-prev">${svgIcon('refresh')}上一步</button>
                  <button class="${footerBtnBase} border-[rgba(88,116,154,0.28)] bg-[rgba(24,33,47,0.92)] px-3 text-[#dbe5f4]" type="button" data-action="wizard-save-draft">${svgIcon('save')}保存草稿</button>
                </div>
                <div class="flex flex-nowrap gap-2">
                  ${step < 3 ? `<button class="${footerBtnPrimary} bg-gradient-to-b from-[#4caeff] to-[#428fe0] px-3.5" type="button" data-action="wizard-next">${svgIcon('arrow')}下一步</button>` : ''}
                  <button class="${footerBtnPrimary} bg-gradient-to-b from-[#62d89a] to-[#48bb7c] px-3.5 ${canStart ? '' : 'opacity-45 pointer-events-none'}" type="button" data-action="wizard-start" ${canStart ? '' : 'disabled'}>${svgIcon('play')}开始游戏</button>
                </div>
              </div>
            </div>
  ...
  function handleClick(event) {
    ...
    switch (action) {
      ...
      case 'open-templates':
        ...
        break;
      case 'open-wizard':
        openWizard(state.selectedTemplateId || activeTemplate()?.id || builtinTemplates[0].id);
        break;
